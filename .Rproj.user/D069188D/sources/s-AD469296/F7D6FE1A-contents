...
bike_orderlines_joined_tb <- orderlines_tbl %>%
    left_join(bikes_tbl, by = c("product.id" = "bike.id")) %>%
    left_join(bikeshops_tbl, by = c("customer.id" = "bikeshop.id"))

glimpse(bike_orderlines_joined_tb)


# 5.0 Wrangling Data ----

bike_orderlines_wrangled_tbl <- bike_orderlines_joined_tb %>%
    ...
    bind_cols(bike_orderlines_joined_tb %>% select(order.id)) %>%
...
sales_by_year_cat_2_tbl <- bike_orderlines_wrangled_tbl %>%
    ...
    # Selecting columns and adding a year
    select(order_date, total_price, category_2) %>%
    mutate(year = year(order_date)) %>%
    
    # Group by year and category_2
...
sales_by_year_cat_2_tbl %>%
    ...
    scale_y_continuous(labels = scales::dollar) +
    labs(
        title = 'Revenue by Year and Category_2', 
